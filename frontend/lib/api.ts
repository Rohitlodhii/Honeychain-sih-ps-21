...
export const authAPI = {
  register: (data: { name: string; phone: string; password: string; role: 'beekeeper' | 'cooperative_admin'; cluster?: string; email?: string; admin_invite_code?: string }) => apiClient.post('/api/auth/register', data),
...
export const hiveAPI = {
  create: (data: HiveCreatePayload) => apiClient.post('/api/hives', data),
  ...
  createReading: (hiveId: string, data: SensorReadingPayload) => apiClient.post(`/api/hives/${hiveId}/readings`, data),
  simulate: (hiveId: string, anomaly?: boolean) =>
    apiClient.post(`/api/hives/${hiveId}/simulate?anomaly=${anomaly || false}`),
}

export const batchAPI = {
  create: (data: BatchCreatePayload) => apiClient.post('/api/batches', data),
  ...
  addEvent: (batchId: string, data: { event_type: string; payload: Record<string, unknown> }) => apiClient.post(`/api/batches/${batchId}/events`, data),
}
